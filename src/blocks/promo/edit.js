...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	const deskStyles = `
		...
		.${uniqueId} .subtitle{
			...
			background-color: ${subtitleBgColor ? subtitleBgColor : ''};
		}

		.${uniqueId} .trawa-do{
			color: ${trawaDoColor ? trawaDoColor : ''};
		}
			...
			<div {...blockProps}>
				...
				<div className="content">
					<RichText
						...
						placeholder={__('title..', 'aff-blocks')}
					/>
					<RichText
						tagName={subtitleTag}
						className="subtitle"
					...
					<h5 className="trawa-do">
						<span>{__('trwa do: ', 'aff-blocks')}</span>
						<RichText
							tagName="span"
							value={trawaDo}
							onChange={(v) => setAttributes({ trawaDo: v })}
							placeholder={__('01.o9.2023', 'aff-blocks')}
						/>
					</h5>
					<RichText
						tagName="p"
